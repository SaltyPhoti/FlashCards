...
在“管理”页面点击“重置本端数据”，可以把当前浏览器或已安装的 PWA 恢复到初始状态。

- 重置会删除本端所有卡组、卡片、图片、拼写记录和复习进度，数据无法恢复。
- 建议先点击“先导出备份”，保留一份完整备份。
- 只影响当前浏览器或设备；其他设备中的数据不会被删除。
- 重置后会重新载入网站当前的 `starter-cards.json`。
